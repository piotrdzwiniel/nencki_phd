...
file_path_2 = 'data/ief/periorbital/cec_gm_wm_eb_magnE.nii.gz'  # Replace with your file path
...
plt.imshow(stacked_slices.T, cmap='jet', origin='lower', alpha=0.5, interpolation='nearest', vmin=0, vmax=0.03)  # Adjust alpha for transparency
...
plt.ylim(100)
cbar = plt.colorbar(label='Electric Field Magnitude (V/m)', location='left', fraction=0.025, pad=0.125)
cbar.ax.tick_params(labelsize=fs)  # Increase font size for colorbar ticks
cbar.set_label('Electric Field Magnitude (V/m)', fontsize=fs)  # Increase font size for colorbar label
...
# Save the figure
plt.savefig('Corrigenda Figure 3-4A.png', dpi=300, bbox_inches='tight', transparent=True)
# plt.show()
