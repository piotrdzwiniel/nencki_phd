...
def get_percentage_in_range(unique_values, percentages, min_val=0.6, max_val=1.0):
...
plt.ylim(0, 7)  # Dopasowanie limitu osi Y dla lepszej wizualizacji
# Create yticks from 0 to 5, but with step of 2.5
plt.yticks([0, 2, 4, 6])
plt.xticks([])
plt.tight_layout()

# plt.savefig('Corrigenda Figure 3-5B Percentages.png', dpi=300, bbox_inches='tight', transparent=True)
plt.show()